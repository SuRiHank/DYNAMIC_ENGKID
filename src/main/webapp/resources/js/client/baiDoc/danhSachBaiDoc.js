$(document).ready(function(){
	...
	function ajaxGet(page){
		...
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/webengkid/api/client/bai-doc/all" + "?page=" + page + "&doKho=" + doKho + "&part=" + part,
				...
				if(result.totalElements > 0){
					...
					$.each(result.content, function(i, baiDoc){
						var html = '<div class="span9">'
							+ '<div class="span3">' 
							+    '<img class="imageExam" src="/webengkid/file/images/baiDocId='+baiDoc.id+'.png" style="width: 300px; height: 150px;"/>'
							+' </div>'
							+ '<div class="span1"></div>'
							+ '<div class="span5"> '
							+    '<h4 class="content-heading" id="namebaithithu">'+ baiDoc.tenBaiDoc + '</h4>'
							+    '<a class="btn btn-primary" href="/webengkid/reading/part-'+baiDoc.part+'/'+baiDoc.id+'"> Chi tiết</button>'
							+  '</div>'
							+ '</div>'
