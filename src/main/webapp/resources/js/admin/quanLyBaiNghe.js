$(document).ready(function() {
	...
	function ajaxGet(page) {
		...
		$.ajax({
			type : "GET",
			url : "http://localhost:8080/webengkid/api/admin/bai-nghe/all" + "?page=" + page + "&doKho=" + doKho + "&part=" + part,
	...
	$(document).on('click', '.btnCapNhatBaiNghe', function(event) {
		...
		var href = "http://localhost:8080/webengkid/api/admin/bai-nghe/"+baiNgheId;
...
//            $('#photoBaiNghe').val("http://localhost:8080/webengkid/file/images/baiNgheId="+ baiNghe.id+".png");
            $("img").attr("src", "http://localhost:8080/webengkid/file/images/baiNgheId="+ baiNghe.id+".png");
            $("#previewImage").removeClass("hidden");
            $("#previewAudio").attr("src", "http://localhost:8080/webengkid/file/audio/baiNgheId="+ baiNghe.id+".mp3");
            $("#previewAudio").removeClass("hidden");
            $("#linkExcel").attr("href", "http://localhost:8080/webengkid/file/excel/baiNgheId="+ baiNghe.id+".xlsx");
	...
	$(document).on("click", ".btnXoaBaiNghe", function() {
		...
		if (confirmation) {
			$.ajax({
				type : "DELETE",
				url : "http://localhost:8080/webengkid/api/admin/bai-nghe/delete/" + baiNgheId,
...
//	});
	...
	function saveFunction(formData) {
		...
		$.ajax({
			...
			url : "http://localhost:8080/webengkid/api/admin/bai-nghe/save",
	...
	function updateFunction(formData, id) {
		...
		$.ajax({
			...
			type : "POST",
			contentType : "application/json",
			url : "http://localhost:8080/webengkid/api/admin/bai-nghe/update/" + id,
			enctype : 'multipart/form-data',
			data : formData,
			// prevent jQuery from automatically transforming the data into a
			// query string
