...
$(document).ready(function() {
	...
	function loadAllGrammar(){
		$.ajax({
			dataType : 'json',
			type:'GET',
			url:"http://localhost:8080/webengkid/api/admin/grammar/loadGrammar",
	...
	$('#btnAddNewGrammar').click(function() {
		...
		$.ajax({
				data: formData,
				type:'POST',
				url:"http://localhost:8080/webengkid/api/admin/grammar/save",
...
// delete baiGrammar	
		...
		if(confirm("Bạn muốn xóa bài grammar này?"))
			{
				$.ajax({
					type:'POST',
					url:"http://localhost:8080/webengkid/api/admin/grammar/delete/"+idBaiGrammar,
					success: function(data){
						loadAllGrammar();
	...
	$(document).on('click','.editBaiGrammar',function(event){
		...
		$.ajax({
			type:'GET',
			url:"http://localhost:8080/webengkid/api/admin/grammar/infoGrammar/"+idBaiGrammar,
			success: function(data){
		...
		$('#btnUpdate').click(function() {
			...
			$.ajax({
				data: formData,
				type:'POST',
				url:"http://localhost:8080/webengkid/api/admin/grammar/update",
				enctype : 'multipart/form-data',
			    contentType : false,
			    cache : false,
			    processData : false,
