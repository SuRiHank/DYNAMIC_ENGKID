$(document).ready(function(){
	...
	function ajaxGet(page){
		...
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/webengkid/api/admin/bai-doc/all" + "?page=" + page + "&doKho=" + doKho + "&part=" + part,
...
//					console.log(baiDoc);
	...
	$(document).on('click', '.btnCapNhatBaiDoc', function(event) {
		...
		var href = "http://localhost:8080/webengkid/api/admin/bai-doc/"+baiDocId;
		$.get(href, function(baiDoc) {
            ...
            $("img").attr("src", "http://localhost:8080/webengkid/file/images/baiDocId="+ baiDoc.id+".png");
            $("#previewImage").removeClass("hidden");
            $("#linkExcel").attr("href", "http://localhost:8080/webengkid/file/excel/baiDocId="+ baiDoc.id+".xlsx");
    ...
    $(document).on("click",".btnXoaBaiDoc", function() {		
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/webengkid/api/admin/bai-doc/delete/" + baiDocId,
    ...
    function resetForm(){
     ...
     function saveFunction(formData){    	 
    	 ...
    	 $.ajax({
    			...
    			url : "http://localhost:8080/webengkid/api/admin/bai-doc/save",
 	...
 	function updateFunction(formData, id) {
		...
		$.ajax({
			...
			type : "POST",
			contentType : "application/json",
			url : "http://localhost:8080/webengkid/api/admin/bai-doc/update/" + id,
			enctype : 'multipart/form-data',
			data : formData,
			// prevent jQuery from automatically transforming the data into a
			// query string
